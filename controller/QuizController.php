...
class QuizController extends Controller {
    ...
    public function submit($id_cours = null) {
        ...
        // Si les réponses ne sont pas envoyées sous la forme attendue (answers[]),
        // essayer de récupérer les champs nommés q0, q1, ... depuis le formulaire HTML.
        if (empty($answers)) {
            foreach ($_POST as $k => $v) {
                if (preg_match('/^q(\d+)$/', $k, $m)) {
                    $pos = intval($m[1]);
                    if (isset($questions[$pos])) {
                        // les radios renvoient 0..3 ; on convertit en 1..4 pour correspondre au modèle
                        $answers[] = ['question_id' => $questions[$pos]['id'], 'selected' => intval($v) + 1];
                    }
                }
            }
        }
...
public function result($attempt_id) {
    ...
    // Récupérer le cours lié pour afficher le titre et construire le lien de retour
    $cours = $this->coursModel->getById($attempt['id_chapitre']);

    $data = [
        'cours' => $cours ?: ['titre' => 'Cours inconnu', 'id_chapitre' => 0],
        'score' => intval($attempt['score']),
        'total' => intval($attempt['total_questions']),
        'time_seconds' => intval($attempt['time_seconds'])
    ];
