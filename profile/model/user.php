...
class User {
    ...
    public function resetPasswordWithToken($token, $newPassword) {
        try {
            ...
            // D'abord trouver l'utilisateur par token
    ...
    public function addMissingColumns() {
        try {
            ...
            // Vérifier et ajouter reset_token_hash
            ...
            // Vérifier et ajouter reset_token_expires_at
    ...
    public function debugToken($token) {
        try {
            ...
            if ($result) {
                ...
                // Vérifier tous les tokens dans la base de données pour le débogage
    ...
    /**
     * NOUVELLE MÉTHODE: Vérifie un utilisateur (email verification)
     */
    public function verifyUser($user_id) {
        try {
            // Vérifier d'abord si la colonne 'is_verified' existe
            $checkStmt = $this->pdo->query("SHOW COLUMNS FROM users LIKE 'is_verified'");
            if ($checkStmt->rowCount() == 0) {
                // Si la colonne n'existe pas, on peut juste retourner true
                // car nous stockons le code dans la session
                return true;
            }
            ...
            error_log("PDO Error in verifyUser: " . $e->getMessage());
            return false;
        }
    }

    /**
     * NOUVELLE MÉTHODE: Vérifie si un utilisateur est vérifié par email
     */
    public function isVerified($user_id) {
        try {
            ...
            error_log("PDO Error in isVerified: " . $e->getMessage());
            return false;
        }
    }

    /**
     * NOUVELLE MÉTHODE: Met à jour les informations de vérification
     */
    public function updateVerificationInfo($user_id, $verification_code) {
        try {
            // Stocker le code de vérification dans la base de données
            $sql = "UPDATE users SET verification_code = ?, verification_code_expires = DATE_ADD(NOW(), INTERVAL 15 MINUTE) WHERE id = ?";
            $stmt = $this->pdo->prepare($sql);
            return $stmt->execute([$verification_code, $user_id]);
        } catch (PDOException $e) {
            error_log("PDO Error in updateVerificationInfo: " . $e->getMessage());
            return false;
        }
    }

    /**
     * NOUVELLE MÉTHODE: Vérifie un code de vérification
     */
    public function checkVerificationCode($user_id, $code) {
        try {
            $stmt = $this->pdo->prepare("
                SELECT id 
                FROM users 
            ...
            error_log("PDO Error in checkVerificationCode: " . $e->getMessage());
            return false;
        }
    }

    /**
     * NOUVELLE MÉTHODE: Récupère un utilisateur par code de vérification
     */
    public function findByVerificationCode($code) {
        try {
            ...
            error_log("PDO Error in findByVerificationCode: " . $e->getMessage());
            return false;
        }
    }

    /**
     * NOUVELLE MÉTHODE: Crée une vérification par email
     */
    public function createEmailVerification($user_id, $email, $code) {
        try {
            ...
            error_log("PDO Error in createEmailVerification: " . $e->getMessage());
            return false;
        }
    }
}
