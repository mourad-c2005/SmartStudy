...
if (!isset($_POST['user_id'])) {
    header('Location: ../view/back_office/user.php?error=invalid_form');
    exit();
}
...
try {
    ...
    // Récupérer les données actuelles de l'utilisateur - CORRECTION ICI
    $currentUser = $userModel->find($user_id);
    
    if (!$currentUser) {
        header('Location: ../view/back_office/user.php?error=user_not_found');
        exit();
    }
